...
const Project = ({props}) => {
  const {title, description, path, source} = props
  return (
        <li className="post underlined align-items-center justify-between">
          <div>
            <h2>
              <a href={path || source}
                target="_blank"
          ...
          {source && 
            <a className='btn btn--primary d-flex align-items-center' href={source}target="_blank" rel="noreferrer">
              <svg className="btn__icon" height="20" viewBox="0 0 16 16" version="1.1" width="20" aria-hidden="true">
